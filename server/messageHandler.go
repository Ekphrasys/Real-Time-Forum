...
func LastMessageHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
	...
	lastTimestamp, err := database.GetLastMessageTimestamp(currentUserID, otherUserID)
	if err != nil {
	...
	if !lastTimestamp.IsZero() {
		response["timestamp"] = lastTimestamp.Format(time.RFC3339)
	}
